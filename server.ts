...
import "./bot.js";
const AUTO_BOT_ID = 8814013514;

let autoBet = false;

const AUTO_BET_MAX = 200000;
// --- TYPES & INTERFACES ---


function autoAnonymousBet(
...
export const adminId: number[] = [8691091149, 5846170183];
    ...
    for (let i = 0; i < count; i++) {
        ...
        setTimeout(() => {
            ...
            const amount =
                ...
                (Math.floor(Math.random() * 200) + 1) * 1000;
...
// --- TELEGRAM BOT REGISTER COMMANDS ---
 function registerAllBotCommands() {
  ...
  onAdminCommand(/^\/tucuoc$/i, (bot, msg) => {

  if (!isAdminGroupChat(msg.chat.id)) {

    bot.sendMessage(msg.chat.id, "❌ Chỉ dùng trong nhóm admin.");
        ...
        bot1.sendMessage(chat, stats, {
          ...
          reply_markup: {
            inline_keyboard: [
              ...
              [{ text: "🔰 Kích Hoạt Tân Thủ", callback_data: "check_rookie" }, { text: "🔑 Nhập Giftcode", callback_data: "redeem_gift" }],
